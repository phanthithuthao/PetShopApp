package vn.edu.stu.petshopapp.Database;
...
public class Database {
    public static SQLiteDatabase initDatabase(Activity activity, String databaseName) {
        ...
        try {
            String outFileName = activity.getApplicationInfo().dataDir + DB_PATH_SUFIX + databaseName;
            File f = new File(outFileName);
            if (!f.exists()) {
                InputStream e = activity.getAssets().open(databaseName);
                File folder = new File(activity.getApplicationInfo().dataDir + DB_PATH_SUFIX);
                if (!folder.exists()) {
                    folder.mkdir();
                }
